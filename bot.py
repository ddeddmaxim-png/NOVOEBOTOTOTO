...
# ВПИШИТЕ СВОЙ TELEGRAM ID СЮДА (можно узнать у бота @userinfobot)
ADMIN_ID = 123456789  
...
# --- ПОКУПКА ТАРИФОВ (ДЛЯ ПОЛЬЗОВАТЕЛЕЙ) ---
...
async def buy_tariff(message: types.Message):
    ...
    # Выводим список тарифов в виде инлайн-кнопок
    inline_kb = []
...
async def select_payment_method(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(f"📊 You selected: **{name}**\nChoose your payment method:", parse_mode="Markdown", reply_markup=inline_kb)
...
async def process_pay_crypto(callback: types.CallbackQuery):
    ...
    # Демо-линк. Для реальных оплат здесь вызывается API Криптобота
...
async def admin_add_tariff(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await callback.message.answer("📝 Enter the **Name** of the new tariff (e.g., VIP Monthly):")
...
async def admin_name_received(message: types.Message, state: FSMContext):
    ...
    await message.answer("🌟 Enter the price in **Telegram Stars** (integer number, e.g., 150):")
...
async def admin_stars_received(message: types.Message, state: FSMContext):
    ...
    await message.answer("⚡ Enter the price in **USD/USDT** for CryptoBot (can be decimal, e.g., 4.99):")
...
async def admin_crypto_received(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("❌ Please enter a valid number (e.g., 5.00 or 4.99):")
        return
    
    user_data = await state.get_data()
    
    # Сохраняем тариф в базу данных
    conn = sqlite3.connect("tariffs.db")
    cursor = conn.cursor()
    cursor.execute(
    ...
    await state.clear()
    await message.answer(f"✅ Tariff **{user_data['name']}** successfully added!", reply_markup=get_main_menu(message.from_user.id))
