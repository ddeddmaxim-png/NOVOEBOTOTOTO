...
# Замените эти цифры на свой ID, иначе кнопка Admin Panel не появится!
ADMIN_ID = 123456789  
...
async def cmd_start(message: types.Message, state: FSMContext):
    # Принудительно очищаем старые зависшие состояния FSM, чтобы кнопки не мертвели
    await state.clear()
...
# --- ОПЛАТА ЧЕРЕЗ CRYPTOBOT (СТАБИЛЬНАЯ ВЕРСИЯ С ОБХОДОМ SSL) ---
...
async def process_pay_crypto(callback: types.CallbackQuery):
    ...
    if not CRYPTO_BOT_TOKEN:
        await callback.message.answer("❌ CryptoBot Token is missing in host variables!")
        await log_to_admin("⚠️ CRYPTO_BOT_TOKEN is not set in Environment Variables!")
        return

    headers = {"Crypto-Pay-API-Token": str(CRYPTO_BOT_TOKEN).strip()}
    payload = {"asset": "USDT", "amount": str(price_usd)}

    # Отключаем строгую проверку SSL хостинга, чтобы избежать ошибки UNRECOGNIZED_NAME
        ...
        try:
            async with session.post(url, headers=headers, json=payload) as response:
                if response.status != 200:
                    ...
                    await callback.message.answer(f"❌ CryptoBot API Server error code {response.status}")
                    await log_to_admin(f"⚠️ CryptoBot API Error {response.status}: {error_text}")
                    return
                ...
                if result.get("ok") is True:
                    ...
                    await log_to_admin(f"⚡ User <b>@{callback.from_user.username}</b> created CryptoBot invoice #{invoice_id} for ${price_usd}")

                    crypto_kb = InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text=f"💸 Pay ${price_usd} via CryptoBot", url=pay_url)],
                        [InlineKeyboardButton(text="✅ Check Payment", callback_data=f"check_{invoice_id}_{tariff_id}")]
                    ])
                    await callback.message.answer("Invoice link created! Pay it in CryptoBot and click check below.", reply_markup=crypto_kb)
                else:
                    description = result.get("description", "Unknown API Error")
                    await callback.message.answer(f"❌ CryptoBot API Error: {description}")
                    await log_to_admin(f"⚠️ CryptoBot rejected request: {description}")
                    
        except Exception as e:
            logging.error(f"CryptoBot direct connection error: {e}")
            await callback.message.answer("❌ Error connecting to CryptoBot. Please try again later.")
            await log_to_admin(f"⚠️ Connection crash: {str(e)}")

@dp.callback_query(F.data.startswith("check_"))
async def check_crypto_payment(callback: types.CallbackQuery):
    data_split = callback.data.split("_")
    invoice_id = int(data_split[1])
            ...
            async with session.post(url, headers=headers, json=payload) as response:
                ...
                if result.get("ok") and result["result"]["items"]:
                    ...
                    if status == 'paid':
                        ...
                        await log_to_admin(f"✅ <b>CRYPTO SUCCESS!</b> User <b>@{callback.from_user.username}</b> paid invoice #{invoice_id}")
                        await callback.message.edit_text("🎉 Thank you! Your payment is confirmed. Tariff activated.")
                    elif status == 'active':
                        await callback.answer("⏳ Invoice is not paid yet. Please pay inside CryptoBot first.", show_alert=True)
                    else:
                        await callback.answer("❌ Invoice expired or cancelled.", show_alert=True)
                else:
                    await callback.answer("Error checking status.", show_alert=True)
        except Exception as e:
            logging.error(f"CryptoBot check error: {e}")
            await callback.answer("Connection error. Try again.", show_alert=True)
...
async def admin_crypto_received(message: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect("tariffs.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO tariffs (name, price_stars, price_crypto) VALUES (?, ?, ?)",
                   (user_data['name'], user_data['stars'], crypto_price))
    conn.commit()
    conn.close()
    
    await state.clear()
    await message.answer(f"✅ Tariff <b>{user_data['name']}</b> added!", parse_mode="HTML", reply_markup=get_main_menu(message.from_user.id))
...
async def admin_delete_list(callback: types.CallbackQuery):
    ...
    delete_kb = [[InlineKeyboardButton(text=f"🗑 Delete {t[1]}", callback_data=f"del_{t[0]}")] for t in tariffs]
